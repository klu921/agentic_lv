...
async def query_model_iterative_with_retry(model, question, uid, vid_path, output_file, max_retries=15):
    ...
    for attempt in range(max_retries):
        try:
            print(f"Attempt {attempt + 1}/{max_retries} for question: {question[:50]}...")
    ...
    for i in range(model.max_num_iterations):
        ...
        try:
            if parsed_response.get("tool") == "FINAL_ANSWER":
                ...
                retrieved_info = await search_captions(question_uid, search_query, f"{vid_path}/captions/frame_captions_sorted_embeddings.jsonl", 30)
