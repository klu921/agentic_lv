...
async def search_captions(uid, query, embeddings_path, topk = 30):
    safe_write_single('embed_queries.json', {"uid": uid, "query": query})
    await asyncio.sleep(2)
    query_emb = await wait_embedding(uid, query, "openai", "text-embedding-3-large", True)
