...
async def query_llm_async(model, prompt, max_tokens=4096, temperature=0.7, max_attempts = 10):
    """
    Async query any open-source model w/ Together AI with timeout
    """
    # Create a new async client for this request to avoid session issues
    async_client = AsyncTogether(api_key=together_key_PRIV)
    try:
        for i in range(max_attempts):
            try:
                response = await asyncio.wait_for(
                    ...
                    timeout=120  # 60 second timeout per API call
                )
                return str(response.choices[0].message.content)
            except asyncio.TimeoutError:
                print(f"Timeout querying {model} after 120 seconds (attempt {i+1}/{max_attempts})")
                continue
            except Exception as e:
                print(f"Error querying {model}: {e} (attempt {i+1}/{max_attempts})")
                print(f"Error type: {type(e).__name__}")
                if "401" in str(e) or "unauthorized" in str(e).lower():
                    print("API Key authentication failed - check your Together API key")
                continue
    except Exception as e:
        pass
